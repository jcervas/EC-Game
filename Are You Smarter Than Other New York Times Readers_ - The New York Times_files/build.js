...
define('pushData',[
  ...
  //dont know if this matters http://attribute.adm.int.newsdev.net/projects/number-guessing-game
    ...
    // godZown.setHost("http://localhost.nytimes.com:4567/"); // for local development
...
define('amanda',['d3/3'], function(d3) {
    ...
    return function(user, everyone, winner, totals, avg, sumOfAll, cont) {
        ...
        var minNumberPeopleOutsmarted = commify(totals.length - totals.filter(function(d) { return d === winner; }).length);
...
//And you weren’t far off from reality if you made that assumption.
        ...
        var sentences = {
            ...
            "within10": "<p>Not bad – but you're not quite one of the winners.  So far, the average of all numbers has been "+avg+", which means two-thirds of the average is "+winner+". About "+d3.round(100 - pctile)+" percent of readers guessed closer than you.</p>",
    ...
    function getPercentileOfScore(userDiff, totals, winner) {
        ...
        diffs = diffs.sort(function(a,b) { return a-b; });
...
require([
    ...
    var xAxisFormat = function(d) { 
        if (prettySurePhone) { return d%10 === 0 ? d : ""; }
        return d%5 === 0 ? d : "";
    };
    ...
    var userResultsUrl = "https://int.nyt.com/newsgraphics/2015/2015-08-11-number-guessing-game/results.json",  // <-- production
    ...
    var a = generateQAPairs(".g-option-1", ".g-answer-1", 50,70);
    var b = generateQAPairs(".g-option-2", ".g-answer-2", 30,50);
    var c = generateQAPairs(".g-option-3", ".g-answer-3", 10,35);


    function generateQAPairs(sel1, sel2, floor, ceil) {
        var dif = ceil - floor,
            qNum = d3.round(floor + Math.random()*dif),
            ansNum = d3.round(2/3 *qNum);
    ...
    function ready(err, seedData, userData) {
        ...
        sumOfAllEntries = d3.sum(gdata, function(d) { return +d.adjFreq; });
        ...
        function checkInputValidity(val) {
            ...
            return  (!isNaN(val2) && (val2 >= 0) && (val2 <= 100) && val2.toString().indexOf("x") < 0);
        }

        function showAnswer() {

            //check validity of answer backup
            var v = $("#g-user-submit-input").val();
            var isValid = checkInputValidity(v);

            if (!isValid) return;

            //deactivate text input
            $("#g-user-submit-input").blur();
            ...
            userGuess = +document.getElementById('g-user-submit-input').value;
        ...
        function render() {
            ...
            var y = d3.scale.linear()
                    .range([height, 0])
                    .domain([0, d3.max(gdata, function(d) { return d.pct; })]);
            ...
            var xAxisGroup = svg.append("g")
                ...
                .call(xAxis)
                .selectAll("g")
                .classed("g-major", function(d) { return d%5 === 0; });
            ...
            var bar = svg.append("g").attr("class", "g-bar-container").selectAll(".g-bar")
                ...
                .enter().append("line")
                .attr("class", "g-bar")
                .classed("g-current-winning-entry", function(d) { return d.guess === winningEntry; })
                .classed("g-user-average", function(d) { return d.guess === averageOfAllGuesses; })
                .classed("g-user-guess", function(d) { return d.guess === userGuess; })
                .attr("transform", function(d) { return 'translate('+x0(d.guess)+',0)'; })
                .attr("stroke-width", x0.rangeBand())
                .attr("y2", function(d) { return y(d.pct) - 1; })
                .attr("y1", y(0)-1);

            if (userGuess) {
                var val = gdata.filter(function(d) { return d.guess === userGuess; })[0].pct;
            ...
            var labels = [
                {"desktopDisplay": "Your guess", "mobileDisplay": "Your guess", "guess": userGuess, "labelClass": "g-user-guess"},
                {"desktopDisplay": "Winner (2/3 of average)", "mobileDisplay": "Winner (2/3 of avg.)", "guess": winningEntry, "labelClass": "g-current-winning-entry"},
                {"desktopDisplay": "Average of all guesses", "mobileDisplay": "Avg. of everyone", "guess": averageOfAllGuesses, "labelClass": "g-user-average"},
            ];

            labels.sort(function(a,b) { return a.guess - b.guess; });
            ...
            function renderFeedbackTable() {
                ...
                var row = kc.append("table").selectAll(".g-key-item-group g-row")
                    ...
                    .enter().append("tr")
                        ...
                        .classed("g-you", function(d) { return d.labelClass === "g-user-guess";});

                var keyTableCell = row.append("td")
                    .attr("class", "g-cell");

                keyTableCell.append("span")
                    .attr("class", function(d) { return d.labelClass + " g-key-block"; });

                keyTableCell.append("span")
                    .attr("class", "g-key-label")
                    .text(function(d) { return mobile ? d.mobileDisplay : d.desktopDisplay; });
